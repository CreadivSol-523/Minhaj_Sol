...
const Header = ({ fixedHead }: { fixedHead?: boolean }) => {
  ...
  return (
    <>
      ...
      <div className={`h-screen w-80 bg-white fixed  top-0  rounded-tl-lg rounded-bl-2xl px-5 py-9 flex flex-col gap-10 transition-all duration-500  ${isSidebar ? "z-50 right-0 " : "-right-full z-0"}`}>
        <div className="flex items-center justify-between">
          <Image src={"/Images/Logo.png"} alt="Logo" width={80} height={80} />
          <CircleX className="cursor-pointer" onClick={() => setIsSidebar(false)} />
        </div>
        <nav className="lg:hidden flex flex-col ">
          <Link href="/" className="text-lg border-b py-5 text-textColor hover:text-textBlue transition fontJakartaSemiBold">
            Home
          </Link>
          <Link href="/about-us" className="text-lg border-b py-5 text-textColor hover:text-textBlue transition fontJakartaSemiBold">
            About
          </Link>
          <Link href="/service/123" className="text-lg border-b py-5 text-textColor hover:text-textBlue transition fontJakartaSemiBold">
            Services
          </Link>
          <Link href="#" className="text-lg border-b py-5 text-textColor hover:text-textBlue transition fontJakartaSemiBold">
            Resources ▼
          </Link>
          <Link href="/contact-us" className="text-lg border-b py-5 text-textColor hover:text-textBlue transition fontJakartaSemiBold">
            Contact
          </Link>
        </nav>
